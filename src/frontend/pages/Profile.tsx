...
import { User, Bookmark, Clock, Settings, LogOut, ChevronRight, Award, BookOpen, Video, Sparkles, FolderKanban, FileEdit, Plus, Trash2, Share2, Save, Loader2 } from "lucide-react";
...
const Profile = () => {
    ...
    const { notes, projects, bookmarks, loading, addNote: addNoteHook, deleteNote, updateNote, addBookmark, updateProjectProgress } = useScholarData();
    ...
    const handleAddNote = async () => {
        ...
        await addNoteHook({ title: newNoteTitle, content: "Start writing your research notes here...", type: 'note' });
        setNewNoteTitle("");
                    ...
                    <div className="lg:col-span-1">
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="rounded-3xl border border-border bg-card p-6 shadow-sm">
                            ...
                            <div className="mt-10 space-y-1">
                                {[
                                    { key: "library" as const, icon: Bookmark, label: "My Library" },
                                    { key: "projects" as const, icon: FolderKanban, label: "Research Projects" },
                                    { key: "notes" as const, icon: FileEdit, label: "Lab Notes" },
                                ].map(tab => (
                                    <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                                        className={`flex w-full items-center justify-between rounded-xl px-4 py-3 text-sm font-bold transition-all ${activeTab === tab.key ? 'bg-primary/10 text-primary shadow-sm border border-primary/10' : 'text-muted-foreground hover:bg-secondary'}`}>
                                        <div className="flex items-center gap-3"><tab.icon className="h-4 w-4" /> {tab.label}</div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                            <div className="flex items-end justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-muted-foreground">All your research data is securely stored and synced across devices.</p>
                                </div>
                            </div>
                                    ...
                                    {activeTab === 'library' && (
                                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                                            <div className="grid grid-cols-3 gap-4">
                                                ...
                                                <div className="rounded-2xl border border-border bg-card p-4 text-center">
                                                    <Video className="h-5 w-5 mx-auto text-destructive" />
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="grid gap-3">
                                                    ...
                                                    {bookmarks.length === 0 && (
                                                        <p className="text-xs text-muted-foreground italic text-center py-4 bg-secondary/10 rounded-xl">No items bookmarked yet. Explore laureates, papers, and lectures to start building your library.</p>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="mt-12 rounded-3xl bg-gradient-to-br from-primary/10 to-transparent p-12 text-center border border-primary/10">
                                                ...
                                                <p className="mt-2 text-sm text-muted-foreground max-w-sm mx-auto">Discover more groundbreaking achievements.</p>
                                        ...
                                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex gap-2">
                                                    <Input className="h-9 rounded-xl bg-secondary/50 border border-border w-48 text-xs" placeholder="Note title..." value={newNoteTitle} onChange={(e) => setNewNoteTitle(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddNote()} />
                                            ...
                                            <div className="grid gap-4">
                                                {notes.length === 0 ? (
                                                    <div className="text-center py-20 bg-secondary/20 rounded-3xl border-2 border-dashed border-border text-muted-foreground italic">
                                                        No notes yet. Create notes here or from the Scientific Lab workspace.
                                                    </div>
                                                ) : (
                                                    notes.map(note => (
                                                        <div key={note.id} className="group relative rounded-2xl border border-border bg-card p-6 transition-all hover:border-primary/30">
                                                            <div className="flex justify-between items-start mb-2">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <span className="text-[9px] uppercase font-bold px-2 py-0.5 rounded-full bg-secondary text-muted-foreground">{note.type}</span>
                                                                </div>
                                                            ...
                                                            {editingNote === note.id ? (
                                                                <div className="space-y-2">
                                                                    <Textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="rounded-xl text-sm" rows={5} />
                                                                    <div className="flex gap-2">
                                                                        <Button size="sm" className="rounded-xl" onClick={() => handleSaveEdit(note.id)}><Save className="h-3 w-3 mr-1" /> Save</Button>
                                                                        <Button size="sm" variant="ghost" className="rounded-xl" onClick={() => setEditingNote(null)}>Cancel</Button>
                                                                    </div>
                                                                </div>
                                                            ) : (
                                                                <>
                                                                    ...
                                                                    <div className="mt-4 flex gap-2">
                                                                        <Button variant="ghost" size="sm" className="h-7 text-[10px] font-bold uppercase text-primary" onClick={() => { setEditingNote(note.id); setEditContent(note.content); }}>Edit Full Text</Button>
                                                                    </div>
                                                                </>
                                                            )}
                                                        </div>
                                    ...
                                    {activeTab === 'projects' && (
                                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <Button size="sm" className="rounded-xl h-9" onClick={() => navigate("/scientific-skills")}>
                                                ...
                                                {projects.map(proj => (
                                                    <div key={proj.id} className="group rounded-3xl border border-border bg-card p-6 transition-all hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5">
                                                        ...
                                                        <div className="mt-6 flex gap-2">
                                                            <Button variant="default" className="flex-1 rounded-xl font-bold h-9 text-xs" onClick={() => {
                                                                updateProjectProgress(proj.id, Math.min(100, proj.progress + 10), proj.progress >= 90 ? 'Completed' : 'Active Research');
                                                                toast.success(`Progress updated to ${Math.min(100, proj.progress + 10)}%`);
